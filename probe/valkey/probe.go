...
func New(config *Config) (*Probe, error) {
	...
	err := c.ForEachShard(context.Background(), func(ctx context.Context, shard *redis.Client) error {
		...
		if err != nil {
			zap.S().Errorf("failed to connect to Valkey instance: %v", shard.Options().Addr)
			return err
		}

		zap.S().Debugf("successfully connected to Valkey instance: %v, result: %v", shard.Options().Addr, res.Val())
